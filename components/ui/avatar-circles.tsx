    ...
    <div className={cn("z-10 flex -space-x-4 rtl:space-x-reverse", className)}>
      {avatarUrls.map((url) => (
        <a
          ...
          aria-label={`${url.name}'s GitHub profile (opens in a new tab)`}
          className="inline-flex min-h-touch min-w-touch items-center justify-center rounded-full"
        >
          <Image
      ...
      {(numPeople ?? 0) > 0 && (
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          aria-label={`View ${numPeople} more contributors (opens in a new tab)`}
        >
          +{numPeople}
        </a>
      )}
