...
export function BgSection() {
  ...
  const onImageFile = async (file: File) => {
    ...
    try {
      ...
      logError(e, { operation: "snippet-generator/bg-image-upload" });
      toast({
          ...
          <>
            ...
            <div>
              <div className="flex items-baseline justify-between">
                <Label id="snippet-gradient-angle-label">Gradient angle</Label>
                <span className="text-[11px] tabular-nums text-muted-foreground" aria-hidden="true">
                  {bg.angle}°
                </span>
              </div>
              <Slider
                aria-labelledby="snippet-gradient-angle-label"
                value={[bg.angle]}
                min={0}
                max={360}
                step={1}
      ...
      <TabsContent value="image" className="space-y-3 pt-3">
        ...
        {bg.kind === "image" ? (
          <>
            ...
            <div>
              <div className="flex items-baseline justify-between">
                <Label id="snippet-image-opacity-label">Background image opacity</Label>
                <span className="text-[11px] tabular-nums text-muted-foreground" aria-hidden="true">
                  {Math.round(bg.opacity * 100)}%
                </span>
              </div>
              <Slider
                aria-labelledby="snippet-image-opacity-label"
          ...
          >
            ...
            <span>
              <strong className="font-medium text-foreground">Choose image</strong> · PNG / JPG /
              WebP, max 5 MB
            </span>
          </button>
        )}
