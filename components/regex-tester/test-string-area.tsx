...
export const TestStringArea = forwardRef<TestStringAreaHandle, TestStringAreaProps>(
  function TestStringArea({ matches, hoveredMatchId, onHoverMatch }, ref) {
    ...
    const handlePaste = useCallback(
      (e: ClipboardEvent<HTMLTextAreaElement>) => {
        ...
        const next = testString.slice(0, start) + pasted + testString.slice(end);
        ...
        if (byteLength(next) > MAX_TEST_BYTES) {
          ...
          toast({
            title: "Paste clipped",
            description: "Pasted content exceeds the 100 KB limit and was trimmed.",
            variant: "destructive",
          });
        }
      },
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          ...
          <FileDropZone
            onText={(text) => {
              const clipped =
                byteLength(text) > MAX_DROP_BYTES ? clipToByteLimit(text, MAX_DROP_BYTES) : text;
              setTestString(clipped);
            }}
          />
        </div>
