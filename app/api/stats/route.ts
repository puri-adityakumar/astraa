...
async function getGitHubStats() {
  ...
  try {
    ...
    const [repoResponse, contributorsResponse] = await Promise.all([
      fetch("https://api.github.com/repos/puri-adityakumar/astraa", { next: { revalidate: 300 } }),
      fetch("https://api.github.com/repos/puri-adityakumar/astraa/contributors?per_page=100", {
        next: { revalidate: 300 },
      }),
    ]);
...
export async function GET() {
  ...
  const { stars, contributors } =
    githubResult.status === "fulfilled" ? githubResult.value : { stars: 0, contributors: 0 };
