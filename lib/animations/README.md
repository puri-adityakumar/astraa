...
function MyComponent() {
  ...
  return (
    <motion.div initial="hidden" animate="show" variants={shouldReduce ? {} : fadeInUp}>
      <h1>Hello World</h1>
    </motion.div>
  );
}
...
```tsx
import { HoverLift, HoverScale, HoverGlow, HoverShine } from "@/components/ui/hover-effects";
...
function List({ items }) {
  ...
  return (
    <motion.ul variants={shouldReduce ? {} : staggerContainer} initial="hidden" animate="show">
      {items.map((item) => (
